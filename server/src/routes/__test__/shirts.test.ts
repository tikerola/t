...
describe("Testing shirts-route", () => {
  it("should respond with all three shirts when there is no filter and 10 products per page", async () => {
    const filter = "";
    const productsPerPage = 10;

    const response = await request(app)
      .get(`/products/shirts/1/?filter=${filter}&ppp=${productsPerPage}`)
      .expect(200);
  ...
  it("should respond with first two shirts when there is no filter and 2 products per page", async () => {
    const filter = "";
  ...
  it("should respond with zero shirts when filter=asdf and 2 products per page", async () => {
    const filter = "asdf";
    const productsPerPage = 2;
